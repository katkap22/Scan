    ...
    static async getListId() {
        try {
            const response = await
                axios({
                    method: 'POST',
                    url: `${API_URL}/api/v1/objectsearch`,
                    headers: {
                ...
                console.log('Ответ от сервера (получение списка ID): ', response.data.items)
                    // response = response.data.items.map(item => item.encodedId);
                    // console.log('Улучшенный ответ', response);
